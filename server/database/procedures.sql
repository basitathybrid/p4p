...
BEGIN
  ...
  DECLARE v_session_locked_until DATETIME DEFAULT NULL;
  ...
  IF v_session_exists > 0 THEN
    SELECT locked_until INTO v_session_locked_until
    FROM signup_sessions WHERE phone = p_phone;

    IF v_session_locked_until IS NOT NULL AND NOW() >= v_session_locked_until THEN
  ...
  ELSE
    ...
    INSERT INTO signup_sessions (
      phone, profile_id, name, email, player_mobile_id, player_id,
      facebook, instagram, telegram, password_hash, otp_code, otp_created_at,
      attempts, locked_until, status
    ) VALUES (
      p_phone, p_profile_id, p_name, p_email, p_player_mobile_id, p_player_id,
...
BEGIN
  ...
  ELSE
    SELECT otp_code, otp_created_at, locked_until, attempts,
           name, email, player_mobile_id, player_id, facebook, instagram, telegram, profile_id, password_hash
      INTO v_otp_code, v_otp_created_at, v_locked_until, v_attempts,
           v_name, v_email, v_player_mobile_id, v_player_id, v_facebook, v_instagram, v_telegram, v_profile_id, v_password_hash
    FROM signup_sessions WHERE phone = p_phone
    FOR UPDATE;
    ...
    ELSE
      INSERT INTO applications (
        ...
        p_phone, v_profile_id, v_name, v_email, v_player_mobile_id, v_player_id,
        v_facebook, v_instagram, v_telegram, 'pending_review', NOW(),
        NULL, NULL, NULL
      )
      ON DUPLICATE KEY UPDATE
        profile_id = v_profile_id, name = v_name, email = v_email,
        player_mobile_id = v_player_mobile_id, player_id = v_player_id, facebook = v_facebook,
